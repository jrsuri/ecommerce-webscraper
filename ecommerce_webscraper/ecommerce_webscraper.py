...
urls = {
        ...
        'https://www.amazon.com.br/s?k=':'Amazon',
        'https://www.terabyteshop.com.br/busca?str=':'TerabyteShop',
        # 'https://www.magazineluiza.com.br/busca/':'Magazine Luiza',
        # 'https://www.americanas.com.br/busca/':'Americanas',
       }
...
def scrape_data(search, url, timeout=45):
    ...
    def target(search, url):
        ...
        try:
            ...
            # Exceção para Amazon (reabre a página se aparecer o erro com imagem de cachorro)
            if site == 'Amazon':
                
                # Encerra o loop assim que a imagem não aparecer mais (faz 5 tentativas)
                for _ in range(5):
                    try:
                        if browser.find_element(By.XPATH, "//img[@alt='Cachorros da Amazon']"):
                            browser.quit()
                            browser = webdriver.Chrome(options=options)
                            browser.get(url + search)
                            sleep(uniform(2, 4))
            ...
            elif site == 'Magazine Luiza':
                product_names = page.find_all('h2', {'data-testid':'product-title'}, limit=limit)
                prices = page.find_all('p', {'data-testid':'price-value'}, limit=limit)
                product_urls = page.find_all('a', {'data-testid':'product-card-container'}, limit=limit)
            
            # A lógica é quase a mesma para todos os sites, exceto Amazon
            elif site == 'Amazon':
                
                # Encontra todas as divs com a classe correspondente
                product_divs = page.find_all('div', class_=re.compile(r'puis-card-container s-card-container'))
                
                product_names = []
                prices = []
                product_urls = []
                
                # Percorre todas as divs encontradas, até o valor definido em 'limit', e verifica se elas possuem preço
                for div in product_divs:
                    
                    # Verifica se as listas já atingiram o limite
                    if len(product_names) >= limit and len(product_urls) >= limit:
                        break
                    
                    # Verifica se existe um span com a classe 'a-price-whole' dentro da div
                    if div.find('span', {'class': 'a-price-whole'}):
                        
                        # Encontra o objeto BeautifulSoup do nome do produto
                        product_name = div.find('h2', {'class': 'a-size-base-plus a-spacing-none a-color-base a-text-normal'})
                        if product_name:
                            product_names.append(product_name)  # Adiciona o objeto BeautifulSoup sem extrair o texto
                            if len(product_names) > limit:
                                continue
                            
                        # Encontra o objeto BeautifulSoup do preço do produto
                        price_whole = div.find('span', {'class': 'a-price-whole'})
                        if price_whole:
                            prices.append(price_whole)  # Adiciona o objeto BeautifulSoup (parte inteira)
                            if len(prices) > limit:
                                continue
                            
                        # Encontra o objeto BeautifulSoup da URL do produto
                        product_url = div.find('a', {'class': 'a-link-normal s-line-clamp-4 s-link-style a-text-normal'})
                        if product_url and 'href' in product_url.attrs:
                            product_urls.append(product_url)  # Adiciona o objeto BeautifulSoup sem processar
                            if len(product_urls) > limit:
                                continue
            
            elif site == 'Americanas':
                product_names = page.find_all('h3', class_=re.compile('product-name'), limit=limit)
                prices = page.find_all('span', class_=re.compile('price__PromotionalPrice'), limit=limit)
                product_urls = page.find_all('a', class_=re.compile('inStockCard__Link'), limit=limit)
            ...
            # Exibe uma mensagem de erro se nenhum produto for encontrado, e fecha o navegador
                ...
                pass
                # print(f'Erro ao ler os dados do site {site} para o item {search_original} / nenhum produto encontrado.')
        
        # Exibe uma mensagem de erro com a linha do erro se ocorrer alguma exceção
        except Exception as e:
            # exc_type, exc_value, exc_traceback = sys.exc_info()
            # print(f'Erro ao ler os dados do site {site} para o item {search_original}.')
            # print(f'Mensagem de erro: {str(e)[:128]}')
